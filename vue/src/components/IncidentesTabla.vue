        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <select
                class="form-select"
                id="mantenimiento-frecuencia"
                v-model="newMantenimiento.frecuencia"
                required
              >
...
const createIncidencia = async () => {
  try {
    ...
    console.log(selectedTipoAveria.value); // <-- Check selection
    ...
    console.log(newIncidenciaToSend); // <-- Check object
...
const createMantenimiento = async () => {
  try {
    if (
      ...
      selectedMantenimientoTaller.value.length === 0 ||
      selectedMantenimientoMachine.value.length === 0
    ) {
      throw new Error('No Machine or Taller Selected');
    }
    ...
    const newIncidencias = selectedMantenimientoMachine.value.map((machine) => ({
      ...
      fecha_ini:
        newMantenimiento.frecuencia === 'Semanal' ? newMantenimiento.fechaSemanal : new Date().toISOString(),
    }));
