<template>
  <div class="container-fluid bg-primary p-5 relative w-full h-screen">
    <h1 class="text-2xl font-bold mx-5 px-3 mb-6">Máquinas</h1>

    <div class="col-md-3 mb-3 mx-5">
      <button class="btn btn-dark mx-3" @click="openModal">
      ...
      <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tbody class="table-light">
            <tr v-for="maquina in displayedMaquinas" :key="maquina.id_maquina" class="cursor-pointer">
    ...
    <div v-if="isToggleStatusModalOpen" class="modal-overlay">
      <div class="modal-container">
        ...
        <div class="modal-footer border-t pt-4 flex justify-end">
          <button type="button" @click="closeToggleStatusModal"
            class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded mr-2">Cancelar</button>
    ...
    <div v-if="isEditMaquinaModalOpen" class="modal-overlay">
      <div class="modal-container">
        ...
        <div class="modal-footer border-t pt-4 flex justify-end">
          <button type="button" @click="closeEditMaquinaModal"
            class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded mr-2">Cancelar</button>
...
const totalPages = computed(() => {
  return Math.ceil(filteredMaquinas.value.length / PAGE_SIZE);
});

// Update displayed machines for the current page
const updateDisplayedMaquinas = () => {
  const start = (currentPage.value - 1) * PAGE_SIZE;
  const end = start + PAGE_SIZE;
  displayedMaquinas.value = filteredMaquinas.value.slice(start, end);
};
...
const handleSearch = () => {
  ...
  if (searchQuery.value) {
    filteredMaquinas.value = maquinas.value.filter(maquina => {
      return maquina.nombre.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        maquina.descripcion.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
        getTallerName(maquina.id_taller).toLowerCase().includes(searchQuery.value.toLowerCase());
    });
  } else {
    filteredMaquinas.value = [...maquinas.value];
  }
  updateDisplayedMaquinas();
};
...
watch(maquinas, (newMaquinas) => {
  filteredMaquinas.value = [...newMaquinas];
  handleSearch();
}, { immediate: true });
watch(filteredMaquinas, () => {
  updateDisplayedMaquinas();
});
watch(currentPage, () => {
  updateDisplayedMaquinas();
