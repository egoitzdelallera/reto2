<template>
  ...
  const toggleStatus = async () => {
    try {
          ...
          const response = await axios.patch(`${API_URL}/${props.user.id_usuario}/toggle-status`,{}, {
              headers: headers
          });
        console.log("Response from toggle status:", response)
          ...
          alertMessage.value = 'Estado del usuario cambiado con exito';
            setTimeout(() => {
              closeModal();
        ...
        console.log("Response data toggle status error:",error.response?.data)
